...
public:
    ...
    void tileCountLimitExceeded(uint64_t limit) override {
        if (tileCountLimitExceededFn) tileCountLimitExceededFn(limit);
    }

    std::function<void (OfflineRegionStatus)> statusChangedFn;
    std::function<void (Response::Error)> responseErrorFn;
    std::function<void (uint64_t)> tileCountLimitExceededFn;
};
...
TEST(OfflineDownload, TileCountLimitExceeded) {
    ...
    test.db.setOfflineTileCountLimit(0);
    ...
    observer->tileCountLimitExceededFn = [&] (uint64_t limit) {
        EXPECT_EQ(0, limit);
        test.loop.stop();
    };
